...
var index_default = {
    async email(message, env2, ctx) {
        ...
        // Tự động lấy domain từ địa chỉ người nhận (ví dụ: test@aqamn.food -> aqamn.food)
        const domain = message.to.split('@')[1];

        // Gửi về đúng domain đó, sử dụng HTTP (vì VPS chưa cài SSL)
        const response = await fetch(`http://${domain}/api/email`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": "leoteoteomailtemp"
            },
            body: JSON.stringify({
        ...
        if (!response.ok) {
            console.error(`Lỗi gửi mail: ${response.status} - ${await response.text()}`);
        }
    }
};
