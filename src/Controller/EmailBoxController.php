...
final class EmailBoxController extends AbstractController
{
    ...
    public function createEmailBox(Request $request): Response
    {
        ...
        $emailBox = $this->createEmailBoxHandler->create($creatorIp, $domain);

        return $this->json(CreateEmailBoxResponseDto::fromEntity($emailBox));
    }

    #[Route('/api/email-box/batch', name: 'api_create_email_box_batch', methods: ['POST'])]
    public function createEmailBoxBatch(Request $request): Response
    {
        $creatorIp = $this->clientIpRetriever->getClientIp($request);
        $payload = $request->toArray();
        ...
        if (empty($emails) || !is_array($emails)) {
            return $this->json(['error' => 'emails array is required'], 400);
        }

        $results = [];
        foreach ($emails as $email) {
            try {
                $emailBox = $this->createEmailBoxHandler->createWithEmail(strtolower(trim($email)), $creatorIp);
                $results[] = ['email' => $emailBox->getEmail(), 'status' => 'ok'];
            } catch (\Exception $e) {
                $results[] = ['email' => $email, 'status' => 'error', 'message' => $e->getMessage()];
            }
        }

        return $this->json(['created' => count(array_filter($results, fn($r) => $r['status'] === 'ok')), 'total' => count($emails), 'results' => $results]);
    }
    ...
    #[Route('/{email}/message', name: 'api_get_email_box_messages_by_address', methods: ['GET'], requirements: ['email' => '.+'])]
    public function getEmailBoxByEmail(string $email): Response
    {
        $temporaryEmailBox = $this->temporaryEmailBoxFetcher->fetchByEmail($email);
        $this->temporaryEmailBoxUpdater->markAsLastAccessedNow($temporaryEmailBox);
    ...
    #[Route('/api/email-box/{email}/email-by-address/{emailUuid}', name: 'api_get_email_box_one_message_by_address', methods: ['GET'], requirements: ['email' => '.+'])]
    public function getOneReceivedEmailMessageByEmail(string $email, Uuid $emailUuid): Response
    {
        $receivedEmail = $this->receivedEmailsFetcher->findOneByTemporaryEmailAddressAndReceivedEmailUuid($email, $emailUuid);
    ...
    #[Route('/email-box/{email}/email-by-address/{emailUuid}', name: 'api_view_email_box_one_message_by_address', methods: ['GET'], requirements: ['email' => '.+'])]
    public function viewOneReceivedEmailMessageByEmail(string $email, Uuid $emailUuid): Response
    {
        $receivedEmail = $this->receivedEmailsFetcher->findOneByTemporaryEmailAddressAndReceivedEmailUuid($email, $emailUuid);
